...
import { ListIdField } from './list-id';

describe('list-id.spec.ts', () => {

    describe('ListIdField', () => {
        it('should construct a field with valid raw data', (done) => {
            const raw   = 'a standard raw char field';
            const field = new ListIdField(raw);
            field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            try {
                const field = new ListIdField(`${STANDARD_DELIMITER}`);
                field.validate();
            } catch (error) {
