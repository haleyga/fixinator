...
import { ISendingTimeField, SendingTimeField } from './sending-time';
...
describe('sending-time.spec.ts', function () {

    this.timeout(10000);

    describe('SendingTimeField', () => {
            ...
            for (let hour = 0; hour <= MAX_HOUR; hour++) {
                for (let minute = 0; minute <= MAX_MINUTE; minute++) {
                    for (let second = 0; second <= MAX_SECOND; second++) {
                        ...
                        const timeField    = new SendingTimeField(`${hourString}:${minuteString}:${secondString}`);
                        timeField.validate();
        ...
        it(`should fail to parse invalid FIX times`, (done) => {
            ...
            let timeField: ISendingTimeField = null;

            try {
                timeField = new SendingTimeField(`24:00:00`);
                timeField.validate();
            } catch (error) {
            ...
            try {
                timeField = new SendingTimeField(`00:350:01`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new SendingTimeField(`00:00:62`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new SendingTimeField(`a`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new SendingTimeField(`4908`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new SendingTimeField(``);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
