...
import { LastSharesField } from './last-shares';

describe('last-shares.spec.ts', () => {

    describe('LastSharesField', () => {
        it('should construct a field with valid raw data', (done) => {
            const id       = 210;
            const idString = '210';

            const field = new LastSharesField(idString);
            field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            try {
                const field = new LastSharesField(`${STANDARD_DELIMITER}`);
                field.validate();
            } catch (error) {
            ...
            try {
                const field = new LastSharesField(`asdf3`);
                field.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                const field = new LastSharesField(``);
                field.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                const field = new LastSharesField(`1.3`);
                field.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
