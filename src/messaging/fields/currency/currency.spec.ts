...
import { CurrencyField, CURRENCY_VALUES } from './currency';

describe('currency.spec.ts', () => {

    describe('CurrencyField', () => {
        it('should construct a field with valid raw data', (done) => {
            for (const value of CURRENCY_VALUES) {
                const field = new CurrencyField(value);
                field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            ...
            for (let i = 0; i < ASCII_UPPER_LIMIT; i++) {
                ...
                if (CURRENCY_VALUES.findIndex((char) => char === ascii) === -1) continue;

                try {
                    const field = new CurrencyField(ascii);
                    field.validate();
                } catch (error) {
