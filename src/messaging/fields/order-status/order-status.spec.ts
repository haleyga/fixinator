...
import { OrderStatusField, ORDER_STATUS_VALUES } from './order-status';

describe('order-status.spec.ts', () => {

    describe('OrderStatusField', () => {
        it('should construct a field with valid raw data', (done) => {
            for (const value of ORDER_STATUS_VALUES) {
                const field = new OrderStatusField(value);
                field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            ...
            for (let i = 0; i < ASCII_UPPER_LIMIT; i++) {
                ...
                if (ORDER_STATUS_VALUES.findIndex((char) => char === ascii) === -1) continue;

                try {
                    const field = new OrderStatusField(ascii);
                    field.validate();
                } catch (error) {
