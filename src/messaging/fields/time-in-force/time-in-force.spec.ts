...
import { TimeInForceField, TIME_IN_FORCE_VALUES } from './time-in-force';

describe('time-in-force.spec.ts', () => {

    describe('TimeInForceField', () => {
        it('should construct a field with valid raw data', (done) => {
            for (const value of TIME_IN_FORCE_VALUES) {
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            ...
            for (let i = 0; i < ASCII_UPPER_LIMIT; i++) {
                ...
                if (TIME_IN_FORCE_VALUES.findIndex((char) => char === ascii) === -1) continue;
