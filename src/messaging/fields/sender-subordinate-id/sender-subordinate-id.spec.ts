...
import { SenderSubordinateIdField } from './sender-subordinate-id';

describe('sender-subordinate-id.spec.ts', () => {

    describe('SenderSubordinateIdField', () => {
        it('should construct a field with valid raw data', (done) => {
            const raw   = 'a standard raw char field';
            const field = new SenderSubordinateIdField(raw);
            field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            try {
                const field = new SenderSubordinateIdField(`${STANDARD_DELIMITER}`);
                field.validate();
            } catch (error) {
