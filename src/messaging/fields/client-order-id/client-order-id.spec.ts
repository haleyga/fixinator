...
import { ClientOrderIdField } from './client-order-id';

describe('client-order-id.spec.ts', () => {

    describe('ClientOrderIdField', () => {
        it('should construct a field with valid raw data', (done) => {
            const raw   = 'a standard raw char field';
            const field = new ClientOrderIdField(raw);
            field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            try {
                const field = new ClientOrderIdField(`${STANDARD_DELIMITER}`);
                field.validate();
            } catch (error) {
