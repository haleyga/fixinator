...
import { DeliveryInstructionsField } from './delivery-instructions';

describe('delivery-instructions.spec.ts', () => {

    describe('AccountField', () => {
        it('should construct a field with valid raw data', (done) => {
            const raw   = 'a standard raw char field';
            const field = new DeliveryInstructionsField(raw);
            field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            try {
                const field = new DeliveryInstructionsField(`${STANDARD_DELIMITER}`);
                field.validate();
            } catch (error) {
