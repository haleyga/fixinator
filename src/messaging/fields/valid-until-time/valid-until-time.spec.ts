...
import { IValidUntilTimeField, ValidUntilTimeField } from './valid-until-time';
...
describe('valid-until-time.spec.ts', function () {
    ...
    describe('ValidUntilTimeField', () => {
            ...
            for (let hour = 0; hour <= MAX_HOUR; hour++) {
                for (let minute = 0; minute <= MAX_MINUTE; minute++) {
                    for (let second = 0; second <= MAX_SECOND; second++) {
                        ...
                        const timeField    = new ValidUntilTimeField(`${hourString}:${minuteString}:${secondString}`);
                        timeField.validate();
        ...
        it(`should fail to parse invalid FIX times`, (done) => {
            ...
            let timeField: IValidUntilTimeField = null;

            try {
                timeField = new ValidUntilTimeField(`24:00:00`);
                timeField.validate();
            } catch (error) {
            ...
            try {
                timeField = new ValidUntilTimeField(`00:350:01`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new ValidUntilTimeField(`00:00:62`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new ValidUntilTimeField(`a`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new ValidUntilTimeField(`4908`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new ValidUntilTimeField(``);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
