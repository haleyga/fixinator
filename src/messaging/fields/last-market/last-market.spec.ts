...
import { LastMarketField, LAST_MARKET_VALUES } from './last-market';

describe('last-market.spec.ts', () => {

    describe('LastMarketField', () => {
        it('should construct a field with valid raw data', (done) => {
            for (const value of LAST_MARKET_VALUES) {
                const field = new LastMarketField(value);
                field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            ...
            for (let i = 0; i < ASCII_UPPER_LIMIT; i++) {
                ...
                if (LAST_MARKET_VALUES.findIndex((char) => char === ascii) === -1) continue;

                try {
                    const field = new LastMarketField(ascii);
                    field.validate();
                } catch (error) {
