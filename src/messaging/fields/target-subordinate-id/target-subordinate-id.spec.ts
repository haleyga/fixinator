...
import { TargetSubordinateIdField } from './target-subordinate-id';

describe('target-subordinate-id.spec.ts', () => {

    describe('TargetSubordinateIdField', () => {
        it('should construct a field with valid raw data', (done) => {
            const raw   = 'a standard raw char field';
            const field = new TargetSubordinateIdField(raw);
            field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            try {
                const field = new TargetSubordinateIdField(`${STANDARD_DELIMITER}`);
                field.validate();
            } catch (error) {
