...
import { AllocationAccountField } from './allocation-account';

describe('allocation-account.spec.ts', () => {

    describe('AllocationAccountField', () => {
        it('should construct a field with valid raw data', (done) => {
            const raw = 'a standard raw char field';
            const field         = new AllocationAccountField(raw);
            field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            try {
                const field = new AllocationAccountField(`${STANDARD_DELIMITER}`);
                field.validate();
            } catch (error) {
