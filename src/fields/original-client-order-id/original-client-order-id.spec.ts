...
import { OriginalClientOrderIdField } from './original-client-order-id';

describe('original-client-order-id.spec.ts', () => {

    describe('OriginalClientOrderIdField', () => {
        it('should construct a field with valid raw data', (done) => {
            const raw = 'a standard raw char field';
            const field         = new OriginalClientOrderIdField(raw);
            field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            try {
                const field = new OriginalClientOrderIdField(`${STANDARD_DELIMITER}`);
                field.validate();
            } catch (error) {
