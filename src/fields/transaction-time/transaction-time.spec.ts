...
import { ITransactionTimeField, TransactionTimeField } from './transaction-time';
...
describe('transaction-time.spec.ts', function () {

    this.timeout(5000);

    describe('TransactionTimeField', () => {
            ...
            for (let hour = 0; hour <= MAX_HOUR; hour++) {
                for (let minute = 0; minute <= MAX_MINUTE; minute++) {
                    for (let second = 0; second <= MAX_SECOND; second++) {
                        ...
                        const timeField    = new TransactionTimeField(`${hourString}:${minuteString}:${secondString}`);
                        timeField.validate();
        ...
        it(`should fail to parse invalid FIX times`, (done) => {
            ...
            let timeField: ITransactionTimeField = null;

            try {
                timeField = new TransactionTimeField(`24:00:00`);
                timeField.validate();
            } catch (error) {
            ...
            try {
                timeField = new TransactionTimeField(`00:350:01`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new TransactionTimeField(`00:00:62`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new TransactionTimeField(`a`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new TransactionTimeField(`4908`);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
            ...
            try {
                timeField = new TransactionTimeField(``);
                timeField.validate();
            } catch (error) {
                chai.expect(error).to.be.an.instanceof(FixinatorParseError);
            }
