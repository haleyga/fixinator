...
describe('possible-resend.spec.ts', () => {

    describe('PossibleResendField', () => {
        it('should construct a field with valid raw data', (done) => {
            const raw = 'a standard raw char field';
            const field         = new PossibleResendField(raw);
            field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            try {
                const field = new PossibleResendField(`${STANDARD_DELIMITER}`);
                field.validate();
            } catch (error) {
