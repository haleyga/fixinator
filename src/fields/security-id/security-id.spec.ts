...
import { SecurityIdField } from './security-id';

describe('security-id.spec.ts', () => {

    describe('SecurityIdField', () => {
        it('should construct a field with valid raw data', (done) => {
            const raw = 'a standard raw char field';
            const field         = new SecurityIdField(raw);
            field.validate();
        ...
        it('should fail to validate a field with invalid raw data', (done) => {
            try {
                const field = new SecurityIdField(`${STANDARD_DELIMITER}`);
                field.validate();
            } catch (error) {
